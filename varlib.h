...
{	/* (1) 상속인 금융거래조회, 요청 송신(K2):"KSNDnnn" File Definition */
	...
	char tr_code			[2];
  	...
  	char code_req			[16];
  	char zip_req			[16];
  	char address_req		[208];
  	char phone_req			[32];
  	char mobile_req			[32];
  	char email_req			[64];
  	char name_inherit		[30]; 
  	char code_inherit		[16];
  	char zip_inherit		[16];
  	char address_inherit	[208];
  	char phone_inherit		[32];
  	char mobile_inherit		[32];
  	char email_inherit		[64];
  	char name_tgt			[30];
  	char code_tgt			[16];
  	char zip_tgt			[16];
  	char address_tgt		[208];
  	char yymmdd_tgt			[8];
  	char relation_tgt		[2];
  	char gubun_tgt			[2];
